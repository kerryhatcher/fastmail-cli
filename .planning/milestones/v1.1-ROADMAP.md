...
- [x] **Phase 5: CalDAV Foundation & Discovery** - Add calendar auth/discovery primitives and calendar listing models needed by every later phase
- [x] **Phase 6: iCalendar Event Semantics** - Build and verify event parsing/serialization for title, timing, location, description, attendees, recurrence, and reminders
- [x] **Phase 7: Calendar & Event CRUD Transport** - Implement CalDAV collection/event create, update, delete, and detail retrieval with ETag-safe writes
- [x] **Phase 8: CLI Calendar Experience** - Expose calendar and event workflows through CLI commands with default today/week/range listing UX
- [x] **Phase 9: MCP Calendar Surface & Live Validation** - Add GraphQL calendar/event operations for AI agents and verify the full milestone against live Fastmail
- [x] **Phase 10: Explicit Range Contract Closure** - Fix one-bound explicit range handling across shared helpers, CLI, and MCP so explicit filtering behaves consistently
- [x] **Phase 11: CLI Attendee Clearing Parity** - Close the CLI update gap so attendee lists can be explicitly cleared through the terminal surface
...
- [x] 05-01-PLAN.md — CalDAV discovery, calendar collection model, config/auth reuse, and local tests
...
- [x] 06-01-PLAN.md — event model plus RFC 5545 parsing/serialization helpers with fixture-driven tests
...
- [x] 07-01-PLAN.md — collection CRUD plus event get/create/update/delete transport and error mapping
...
- [x] 08-01-PLAN.md — clap surface, range-default UX, and JSON output contracts
...
- [x] 09-01-PLAN.md — GraphQL calendar schema plus live Fastmail validation checklist and evidence capture

### Phase 10: Explicit Range Contract Closure
**Goal**: Explicit event range filters behave predictably across the shared listing helper, CLI, and MCP, including validation of partial explicit-range inputs
**Depends on**: Phase 9
**Requirements**: EVT-03, CLI-02, MCP-02
**Gap Closure**: Closes milestone-audit requirement, integration, and flow gaps around explicit range handling
**Success Criteria** (what must be TRUE):
1. Supplying both `start` and `end` still returns the requested explicit range without regressing default-today or week shortcuts
2. Supplying only `start` or only `end` is handled explicitly rather than silently falling back to `default_today_range()`
3. CLI and MCP both expose the same explicit-range contract because they share the corrected helper behavior
4. Verification covers the one-bound cases that the milestone audit flagged as broken
**Plans:** 1/1 plans complete

- [x] 10-01-PLAN.md — shared explicit-range validation, surface contract alignment, and live CLI/MCP verification

### Phase 11: CLI Attendee Clearing Parity
**Goal**: The CLI update surface can intentionally clear attendees so terminal behavior matches the underlying event patch semantics
**Depends on**: Phase 10
**Requirements**: EVT-08, CLI-01
**Gap Closure**: Closes the milestone-audit integration gap between CLI event update inputs and shared attendee patch semantics
**Success Criteria** (what must be TRUE):
1. CLI update commands provide an explicit way to send an empty attendee list
2. Existing attendee add/replace flows remain intact and unambiguous
3. Verification proves the CLI can clear attendees without requiring lower-layer workarounds
**Plans:** 1/1 plans complete

- [x] 11-01-PLAN.md — explicit CLI attendee-clearing flag, conflict-safe parsing, and live validation
...
|-------|----------------|--------|-----------|
| 5. CalDAV Foundation & Discovery | 1/1 | Complete | 2026-04-03 |
| 6. iCalendar Event Semantics | 1/1 | Complete | 2026-04-03 |
| 7. Calendar & Event CRUD Transport | 1/1 | Complete | 2026-04-03 |
| 8. CLI Calendar Experience | 1/1 | Complete | 2026-04-03 |
| 9. MCP Calendar Surface & Live Validation | 1/1 | Complete | 2026-04-03 |
| 10. Explicit Range Contract Closure | 1/1 | Complete | 2026-04-03 |
| 11. CLI Attendee Clearing Parity | 1/1 | Complete | 2026-04-03 |
